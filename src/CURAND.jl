...
include(joinpath(Pkg.dir("JuCUDA"),"lib/7.5/libcurand.jl"))
include(joinpath(Pkg.dir("JuCUDA"),"lib/7.5/libcurand_types.jl"))

@compat if is_windows()
    const libcurand = Libdl.find_library(
    ["curand64_75", "curand64_70"])
else
    const libcurand = Libdl.find_library(["libcurand"])
end
isempty(libcurand) && error("CURAND library cannot be found.")

function check_curandstatus(status)
    status == CURAND_STATUS_SUCCESS && return nothing
